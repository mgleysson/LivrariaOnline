...
public class Cadastrar {
	...
	/*
	 * TODO
	 * Favor a variável CodigoCliente seria apenas um meio de distinção entre um usuário e
	 * um administrador. Não será mais necessário após as últimas mudanças.
	 */
	...
	public static Usuario pegarUser(String email,char[] senha){
		...
		for(int i=0;i<listaClientes.size();i++){
			if(listaClientes.get(i).getEmail().equals(email) && listaClientes.get(i).getSenha().equals(senha)){
		...
		if(pegou != true){
			for(int i=0;i<listaAdmin.size();i++){
				if(listaAdmin.get(i).getEmail().equals(email) && listaAdmin.get(i).getSenha().equals(senha)){
